...
const FIXTURES = join(import.meta.dir, "testing", "fixtures");
...
describe("end-to-end workflow", () => {
	test("init through project, issue, plan, implement, review, and done", async () => {
		...
		const planNext = spawnDeckJson(
			["issue", "next", "--mode", "plan", "--project", "E2E"],
			env,
		);
		...
		expect(
			await runDeck(
				["issue", "attach-plan", publicId, "--body", PLAN_PATH, "--json"],
				isolated,
			),
		).toBe(0);
		...
		expect(await Bun.file(planFile).text()).toBe(planMarkdown);

		expect(
			await runDeck(["issue", "approve-plan", publicId, "--json"], isolated),
		).toBe(0);
		...
		expect(
			await runDeck(
				["issue", "move", publicId, "--status", "needs-review", "--json"],
				isolated,
			),
		).toBe(0);
		...
		expect(
			await runDeck(
				["issue", "move", publicId, "--status", "accepted", "--json"],
				isolated,
			),
		).toBe(0);
		...
		expect(
			await runDeck(
				["issue", "move", publicId, "--status", "done", "--json"],
				isolated,
			),
		).toBe(0);
	...
	test("simple issue is selectable for implement without a plan", async () => {
		...
		await runDeck(
			["project", "add", "--key", "SIM", "--name", "Simple", "--json"],
			isolated,
		);
		await runDeck(
			[
		...
		const next = spawnDeckJson(
			["issue", "next", "--mode", "implement", "--project", "SIM"],
			env,
		);
